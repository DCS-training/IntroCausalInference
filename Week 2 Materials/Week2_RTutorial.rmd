...
library(glmnet) # For fitting logistic regression models
...
```{r Exercise 2}
X <- model.matrix(treated ~ INSERT COVARAITES HERE, data = penguins)[, -1]
y <- penguins$treated

logit <- cv.glmnet(X, y, family = "binomial", alpha = 0)
...
### Exercise 4
Now we need to perform some sort of matching. We want to do this based on the propensity scores. We can use **nearest-neighbour matching** based on propensity scores. We should then assess balance before and after matching. Comment on what happens after matching.
